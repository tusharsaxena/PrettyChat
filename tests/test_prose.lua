...
-- sweep of that day found 83 lines here, and the shape of them is the argument for a gate rather
-- than a grep: 33 were inside Blizzard's own `GlobalStrings/` dump, which is the game's English
-- arriving whole from the client and none of this addon's business, while the rest sat in authored
-- locale tests, the override module and the schema. Telling those two apart is exactly what a
-- NAMED exclusion list does and a pattern does not.
--
...
local SKIPPED_DIRS = {
    ...
    "docs/reviews/",
    "docs/revendor/",
    -- Blizzard's own generated string dump, not authored prose. It is the GAME's English, it
    -- arrives whole from the client, and a British spelling inside it is a fact about WoW rather
    -- than a defect here -- 33 of the 83 hits the first sweep found were in this one tree. Named
    -- as a directory for the same reason every other exclusion is named rather than inferred.
    "GlobalStrings/",
}
