...
return function(ctx)
    ...
    test("the read-only Original row degrades when the reference dump is absent", function()
        -- GlobalStrings/ is not loaded headlessly, so the panel must fall back
        -- to its localized placeholder rather than showing nil.
        local orig = lootBlocks[1].rows[1].children[2]
        t.eq(orig.text, L["(original not available)"], "the placeholder is shown")
    end)
