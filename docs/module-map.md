...
| `core/Database.lua` | `NS.Database` — `SCHEMA_VERSION`, `global.schemaVersion` default, `RunMigrations(db)` (empty migration set today) and the load-pass repair `PruneOrphans(db)`, which drops `strings` / `disabledStrings` keys that have no schema row and prunes emptied tables. Merged into AceDB + run in `PrettyChat:OnInitialize` and the three profile callbacks. |
