...
| File | Responsibility |
|------|----------------|
| `core/EnvSetup.lua` | `NS.Meta(field)` and `NS.Version()` — the `LibKa0s-Env-1.0` seam, which tells the library which addon FOLDER is asking so a vendored copy can read the right manifest. Falls back to the C_AddOns-then-legacy-global ladder when the library is absent. Loads first among addon files, and that position is load-bearing. |
