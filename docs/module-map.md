...
9. `core/Database.lua` — populates `NS.Database` (`SCHEMA_VERSION`, `global` defaults, `RunMigrations`, `PruneOrphans`). `PruneOrphans` looks rows up through `NS.Schema.FindByPath` at call time, which is after every file has loaded, so loading ahead of `settings/Schema.lua` is safe.
10. `core/PrettyChat.lua` — creates the AceAddon object **from the `NS` table** (`:NewAddon(NS, …)`, architecture-§2), merges `NS.ProfileDefaults` + `NS.Database.defaults` + runs migrations in `OnInitialize`, registers slash commands, and delegates `OpenConfig` to the library's combat-gated panel-open. **Every later file reaches the addon object via** `LibStub("AceAddon-3.0"):GetAddon("PrettyChat")` (which returns `NS`).
...
15. `defaults/Profile.lua` — populates `NS.ProfileDefaults` (the AceDB `profile` defaults table).
...
| `core/Database.lua` | `NS.Database` — `SCHEMA_VERSION`, `global.schemaVersion` default, `RunMigrations(db)` (`SCHEMA_VERSION` 2: one step, v2, lifts a Loot-only `LOOT_ITEM_CREATED_SELF[_MULTIPLE]` override onto Tradeskill — PRETTYCHAT-R-02; a profile-scoped step runs on every stored profile, and the stamp advances only past a step that succeeded) and the load-pass repair `PruneOrphans(db)`, which drops `strings` / `disabledStrings` keys that have no schema row and prunes emptied tables. Merged into AceDB + run in `PrettyChat:OnInitialize` and the three profile callbacks. |
