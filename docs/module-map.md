...
| `core/Database.lua` | `NS.Database` — `SCHEMA_VERSION`, `global.schemaVersion` default, `RunMigrations(db)` (empty migration set today; a profile-scoped step runs on every stored profile, and the stamp advances only past a step that succeeded) and the load-pass repair `PruneOrphans(db)`, which drops `strings` / `disabledStrings` keys that have no schema row and prunes emptied tables. Merged into AceDB + run in `PrettyChat:OnInitialize` and the three profile callbacks. |
