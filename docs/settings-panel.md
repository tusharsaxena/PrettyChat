...
`sink` still **defaults to `NS.Print`**, and the default is still right: a caller with no console — a degraded load where `LibKa0s-DebugLog-1.0` never registered — gets chat rather than nothing. `NS.Print`'s own destination is untouched: the sink is a parameter, not a redirection, which is why the two surfaces cannot drift into two different reports. `tests/test_override.lua` asserts the sunk report is byte-for-byte the printed one, header to footer.
