...
|---|---|---|---|
| `General` | `Master controls` | — | 4 |
| `Categories` | Loot (35), Currency (9), Money (17), Reputation (29), Experience (41), Honor (13), Tradeskill (17), Misc (5) | one per format string: Loot 17, Currency 4, Money 8, Reputation 14, Experience 20, Honor 6, Tradeskill 8, Misc 2 | 166 |
...
`SetRenderer` owns **when** a page draws: its first `OnShow` — at registration time the body's frame width is zero, and AceGUI's `List` layout sizes children against the container's current width, so building too early produces a stack of misaligned widgets — and again after a refresh flagged it dirty while hidden. It also builds the Defaults button on **every** `OnShow` (outside the rendered guard) and owns the **combat lock** (options-ui-§2, LibKa0s v1.46.x): a page shown in combat — or open when combat starts — is **covered** (`ctx.__combatCover`, a dim with the centered gray *Settings are locked during combat.*), nothing renders, and the settings window is **not closed** (nothing of Blizzard's — `SettingsPanel`, `HideUIPanel`, `ToggleGameMenu` — is touched, anti-pattern #88). One gray `COMBAT_LOCKED_NOTICE` chat line is printed per combat, writes, Defaults and tab switches are refused, and `PLAYER_REGEN_ENABLED` takes the cover down and draws the page from current state. The library registers `PLAYER_REGEN_*` only while one of its pages is on screen. There is no host-side combat guard on any settings page, tab or control, and there must not be one beside the library's.
...
`General` is a *virtual category* — no entry in `NS.Defaults`, no per-string rows. It is built by `buildGeneralBody(ctx)`, which hands the page to `H.RenderTabbedSchema` and draws nothing of its own. Its one tab is `Master controls`, and **every control on it is composed, not hand-written**: `H.MasterControls` (`OptionsCompose 7`) owns the row set, its order and its wording, and `settings/Schema.lua` splices the result in at the head of the schema with the stored paths and defaults this addon already shipped.
...
|---------|-------------|---------|
| **Enable PrettyChat** (left) | `General.enabled` | Master switch. Off means the addon **stands down**, not merely that it stops drawing (`slash-commands-§7`): every Blizzard original is restored regardless of per-category settings, and the combat watcher is unregistered rather than gated. The row's own `set` writes the stored path and then drives the latch (`NS.Lifecycle`), so the checkbox, `/pc enable` / `/pc disable` and `/pc set General.enabled false` are one act. |
| **General visibility** (right) | `General.visibility` | The four canonical modes, `Always` / `Only in combat` / `Only out of combat` / `Never`. This addon's *display* is the chat text it rewrites, so the mode rides the same gate as Enable inside `ApplyStrings`: `Never` restores every original, and the two combat modes restore or re-apply at the combat boundary. Honored by `PrettyChat:IsVisible` and `PrettyChat:SyncCombatWatch` in `modules/Override.lua`; stored only when it differs from `always`. |
...
| **Minimap button** | `global.minimap.shown` (**global** store, stored as LibDBIcon's `db.global.minimap.hide`; WS-06) | The launcher's visibility (`launcher-§3`). **The row says SHOWN and the stored key says HIDDEN**, so its `get`/`set` invert in `settings/Schema.lua`'s `MASTER_WIRING`: the stored table is LibDBIcon's OWN `minimap` table, the one it writes `hide` into from its own right-click menu and `minimapPos` into when the player drags the button, and a second `show` key beside it would be one state kept in two records. The `set` writes the LEAF and then calls `NS.Launcher:SetShown`, so the button follows the checkbox immediately rather than at the next reload. **GLOBAL rather than profile**, deliberately: a profile switch must not move a player's buttons. **And the choice survives every reset the addon ships** (`launcher-§3`) — *Reset all settings* below, and a page **Defaults** button — because it is a per-installation display preference like the angle the button was dragged to, not because of where it happens to be stored. Tick it off, reset everything, and the button stays off. **Clicking the button** (`launcher-§2`, Launcher minor 4): left-click opens this panel, in either state; right-click opens a context menu titled **Ka0s Pretty Chat** with one checkbox, **Enabled**, which calls the same handler as `/pc enable` / `/pc disable` (so it writes this page's **Enable PrettyChat** row). No Locked, Test mode or Show window entry: the addon is frameless and has none of those states. **Hovering the button** shows the library's status tooltip (`launcher-§1`, Launcher minor 3), enabled or disabled: `Ka0s Pretty Chat  v<version>`, `Enabled: Yes|No`, `Left-click: Open settings`, `Right-click: Options menu`. No Locked or Test mode line: the addon has neither state. |
...
**The list is AceGUI's `TreeGroup` and not a hand-built column**, and that distinction is the difference between a selector and a column of colored text. The first attempt drew one `InteractiveLabel` per string and marked the selection with the text color alone: no bordered pane, no bar behind the selected row, no row spacing. `TreeGroup` is the container every AceConfig options window with a left nav is made of, so all of that — the border round both panes, the highlight bar, the spacing, and the tree pane's own scrollbar once the list outgrows the height — is the widget's, and this addon draws none of it.
...
The selected format string renders in the **right-hand column** — the `TreeGroup`'s content pane, whatever width the page leaves beside the `TREE_W` (200px) tree. Its **`Heading` is gone** — the list entry that selects the string is its name now:
...
Both the General page's "Test" button and the `/pc test` slash command go through `PrettyChat:TestToConsole(filter)` (in `settings/Panel.lua`), which opens the debug console and calls `PrettyChat:Test(filter, sink)` (in `modules/Override.lua`) with the console's writer. The button passes no filter (every category, every string); the slash dispatcher (`runTest`) forwards `{kind="category", value=…}` or `{kind="formatstring", value=…}` for the subcommand variants. See [slash-dispatch.md](./slash-dispatch.md#command-reference) for the user-facing forms.

**They no longer differ in anything, and that is the point.** `sink` is the output function every line goes through; both callers pass the debug console's writer (`function(line) NS.DebugLog:Add("Test", line) end`) and both open the console first. The full report is 500+ lines with every category enabled, and the chat frame is the thing this addon exists to keep readable — a window with a scrollbar and a Copy button is what a report that long actually needs, and that was as true of the verb as of the button. Until this changed they disagreed: the button opened the console, `/pc test` put eighty-odd lines into chat. One name doing two things.

`sink` still **defaults to `NS.Print`**, and the default is still right: on a degraded load where `LibKa0s-DebugLog-1.0` never registered there is no console, so `TestToConsole` (which resolves that library once at file load) calls `Test(filter)` with no sink and the report goes to chat, every run, rather than into the console stub's no-op `Add`. `tests/test_libka0s.lua` pins this by running `/pc test category Loot` twice with the library absent (PRETTYCHAT-R-06). `NS.Print`'s own destination is untouched: the sink is a parameter, not a redirection, which is why the two surfaces cannot drift into two different reports. `tests/test_override.lua` asserts the sunk report is byte-for-byte the printed one, header to footer.

The function:

1. Walks `Schema.CATEGORY_ORDER` (so output order matches the Categories page's tab strip). Per category, the strings table is sorted alphabetically by global name. The `filter` argument is applied at both layers — a category filter skips non-matching categories before iterating their strings, a formatstring filter is applied per-string and shows the global under the one category that registers it (so `LOOT_ITEM_CREATED_SELF` prints under Tradeskill).
...
4. **Every line — header, category banner, body lines, blank-line separators, footer — goes through the one `sink`.** In the debug console each line is an entry under the `Test` tag; only on the degraded `NS.Print` fallback does it carry the `[PC]` prefix, which keeps the report distinguishable from real chat traffic interleaved with it. Header includes a notice when `IsStoodDown()` is true (any hold on the latch; in production only the player's switch takes one). Footer reports both counts: `"end of test output (N strings shown, K errored)"` (the `K errored` clause is omitted when zero). When the filter matches no strings (e.g. `/pc test category General` — the virtual category has no strings) the function emits `(no matching strings)` and skips the footer.
