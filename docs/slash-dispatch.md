...
While the addon is **disabled**, a verb that **drives its features** refuses on one tagged line naming `/pc enable`, and does nothing else (`slash-commands-§2`). These stay live, always: `help`, `config`, `version`, `enable`, `disable`, `debug`, `perf`, and the schema CLI — `get`, `set`, `list`, `reset`, `resetall`. The reasoning is the player's rather than the addon's: they have to be able to **read and repair settings** and **reach the panel** while the addon is off, which is precisely when they are most likely to need to; `debug` and `perf` are diagnostics, not features; and `enable` above all, or the pair is one-way again.

**It is implemented once, on the `COMMANDS` table's handler slot** (`settings/Slash.lua`, just after the table above), not as a guard inside each verb and not inside a dispatcher. There are *two* dispatchers here — the library's and the degraded stub — and the only thing they agree on is that both call `entry[3]`, so the handler slot is the one seam every verb crosses on both paths. A single loop wraps every entry whose verb is **not** in the `LIVE_WHILE_DISABLED` set, which is the rule written once as data. The consequence is the point: the **default is gated**, and a verb added later opts out by being named rather than by somebody remembering to paste a guard into it.
