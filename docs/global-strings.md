...
If on-demand loading is ever wanted (e.g. to cut startup memory by dropping the eager load), it needs a shared accessor so the chunk data is reachable across namespaces — re-adding a sub-addon TOC alone would not be enough. No re-split is required; the chunk files themselves are unchanged by any of this.
