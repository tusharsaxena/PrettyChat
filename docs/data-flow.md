...
`ApplyStrings` returns `(applied, restored)` counts rather than logging them itself, so a caller that reports them does so in **one** line (debug-logging-§8/§9). Only the profile switch does, as `[Profile] switched → applied N restored M`. A reset logs instead the one `[Set]` line debug-logging-§10 asks for, counting the rows it changed: `[Set] reset <Cat|Cat.NAME>: N rows`, or `[Set] reset profile '<name>' to defaults (N rows)` for `/pc resetall`. A settings change logs only `[Set] <path> = <value>` at the write seam (debug-logging-§10) — the re-apply is implied and not re-echoed. `OnEnable` no longer logs a boot line (the session-only debug flag is off at load, so it would never render); the self-identifying `[Init]` summary rides the `DebugLog:SetEnabled` seam instead (debug-logging-§5). (Loot lines themselves never log: the addon hooks no events; it only swaps `_G[GLOBALNAME]`.)
