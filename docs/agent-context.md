...
- **Cyan `[PC]` chat prefix on all addon output.** Routes through `NS.Print(msg)` in `core/PrettyChat.lua`, which prepends `NS.PREFIX` (`|cff00ffff[PC]|r `). **No raw `print(...)` calls.** `Test()` (in `modules/Override.lua`) also routes every line through `NS.Print`, so each body line carries the `[PC]` prefix — no direct `DEFAULT_CHAT_FRAME:AddMessage` writes. Gated developer logging goes through `NS.Debug(tag, fmt, ...)`, which routes to the on-screen debug console (`core/DebugLog.lua`, `NS.DebugLog`) — zero-alloc no-op when off, gated on session-only `NS.State.debug` (`core/State.lua`), toggled through the single `NS.DebugLog:SetEnabled(on)` seam, driven by `/pc debug on|off` and the console header toggle. Window **visibility** is a separate concern (`NS.DebugLog:Show()`/`:Hide()`/`:Toggle()`/`:IsShown()`), driven by bare `/pc debug` and the General-page **Debug console** checkbox — the window's OnShow/OnHide fire `Schema.NotifyPanelChange("General")` to keep that checkbox in sync. The `SetEnabled` seam (debug-logging-§5) prints a colour-coded chat ack (`debug logging |cff40ff40ON|r` / `|cffff4040OFF|r`, matching the header toggle), writes the `[Debug] logging enabled|disabled` bracket via raw `D:Add`, and on **enable** appends a one-line `[Init]` session summary (`PrettyChat v<ver>, schema v<n>, profile '<key>'`) — the visible boot summary, since the session-only flag is off at login. Coverage traces the main flows (debug-logging-§8/§9/§10) as **one gated line per event**, never per string: `[Init]` (the on-enable summary above), `[Migrate]` (`core/Database.lua`, only when a step runs), `[Set] <path> = <value>` (`Schema.Set`, the single settings-change seam — no `[Apply]` re-echo), `[Reset]` (`modules/Override.lua` `ResetCategory`/`ResetAll`, with apply counts), and `[Config]` (`OpenConfig` — opened/refused/blocked). `ApplyStrings` returns `(applied, restored)` and stays silent so its callers emit the single summary. Add more `NS.Debug("<Tag>", …)` calls at any point worth tracing — one summary line per pass.
...
- `tests/run.lua` loads the addon sources into a mock WoW env (`tests/wow_mock.lua`, `tests/loader.lua`) under stock Lua 5.1 and runs one suite per module: `test_compat`, `test_constants`, `test_util`, `test_locale`, `test_defaults`, `test_schema`, `test_render`, `test_apply`, `test_override`, `test_database`, `test_lifecycle`, `test_debuglog`, `test_slash`, `test_panel`. Each suite registers named `test(name, fn)` cases via `ctx.test`; a case passes when its body neither errors nor trips a failed assertion. `ctx.loadAddon()` returns a **fresh, fully-booted instance** (`{ env, NS, addon }`), so suites never share state — call it again for a clean addon.
- Mock fidelity worth knowing before you extend it (`tests/wow_mock.lua`): frames resolve unknown **PascalCase** keys to a self-returning no-op but unknown lowercase keys to `nil` (addon-owned fields like `panel.defaultsBtn` are lowercase, and a blanket catch-all would invert every `if not …` guard); `Show()`/`Hide()` fire the OnShow/OnHide scripts *and* hooks, which is what makes the panel's lazy first-OnShow build testable; the AceGUI mock records widget type, label, value, disabled, callbacks and child order (fire a callback with `widget:Fire("OnValueChanged", v)`); the Settings API is modelled, so `registerPanels()` really runs at `OnEnable`.
...
The addon object **is** the `NS` table itself — `core/PrettyChat.lua` passes `NS` to `:NewAddon` (architecture-§2), so the AceAddon methods hang off `NS`. Other files still reach it via `LibStub("AceAddon-3.0"):GetAddon("PrettyChat")`, which returns that same table.
