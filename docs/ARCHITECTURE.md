...
| `core/LauncherSetup.lua` | `NS.Launcher` | The `LibKa0s-Launcher-1.0` seam (`launcher-§1`). Builds the **one** LibDataBroker-1.1 object — `type = "launcher"`, the addon's own 128×128 logo, the label a broker display prints — and hands it to LibDBIcon-1.0, so the minimap button and a broker row are one object registered twice rather than two features. Supplies the FOLDER name (LibDBIcon keys the button's saved position by it), the icon path, `openSettings` (`PrettyChat:OpenConfig`, the one combat-gated open) and a **function** answering `db.global.minimap`, resolved at `Register` time because AceDB has not run when this file loads. **No `onClick` is passed, and the absence IS the rung**: `launcher-§2` puts this addon on rung **(c)** — no primary window, and frameless, so no preview switch — which means left-click opens the settings panel exactly as right-click does. **The hover tooltip is the library's** (Launcher minor 3, `launcher-§1`), drawn enabled or disabled: `Ka0s Pretty Chat  v<version>`, `Enabled: Yes|No`, `Left-click: Open settings`, `Right-click: Open settings`. The descriptor answers only the questions this addon has — `version` (the TOC's `## Version`, through `NS.Version()`) and `isEnabled` (`PrettyChat:IsAddonEnabled()`, with the dispatcher's `DisabledLine` beside it as the library requires); on rung (c) that pair gates nothing. It passes no `isLocked` or `isTestMode` (frameless: no lock, no test mode), no `leftClickLabel` (ignored on rung (c)) and no `onTooltipShow` (no line of its own). **It has no degradation stub and needs none**: its only two call sites (`OnEnable`'s `Register`, and the `global.minimap.shown` row's `set`) both guard on `NS.Launcher`, and the row reads and writes the store either way. `Register()` is called from `PrettyChat:OnEnable`, not here. |
...
| `settings/Slash.lua` | `NS.COMMANDS`, `NS.SlashCommands` | The ordered `COMMANDS` table (positional triples, the host's), the `LibKa0s-Slash-1.0` descriptor with its `format` and `parse` hooks, and the five host-owned verbs: `list`'s sub-keywords and category filter, `reset`'s category-misuse redirect, `resetall`, `test`, `debug`. |
...
**What survives, because it is SETUP and not a feature.** The `/pc` and `/prettychat` chat commands and the whole dispatcher; the settings-category registration and its panel body; the AceDB handle, the single write seam and the three profile callbacks; and the launcher's broker object and minimap button. The launcher's **left** click is unrefused: Pretty Chat is the rostered rung-**(c)** addon and `launcher-§2` carves rung (c) out by name, because a rung-(c) left click opens the settings panel and `§7` itself keeps the panel standing. Both buttons open it, in both states, and neither writes SavedVariables. The hover tooltip stays too, and reads `Enabled: No`.
...
Vendored under `libs/` (the BigWigs packager pulls nothing — no `externals`): LibStub, CallbackHandler-1.0, AceAddon-3.0, AceDB-3.0, AceConsole-3.0, AceGUI-3.0, **LibDataBroker-1.1** and **LibDBIcon-1.0** (the launcher's pair, `launcher-§1` — neither is LibKa0s's dependency; it resolves both with `LibStub(…, true)` at `Register` time and degrades by name), and **[LibKa0s](https://github.com/tusharsaxena/LibKa0s) v1.57.0** (`libs/LibKa0s/`, listed in the TOC as `libs\LibKa0s\LibKa0s.xml` after Ace3). (`AceConfig-3.0` was removed — no live consumer.)
