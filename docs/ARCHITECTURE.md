...
- `global.minimap.hide` — the minimap button's visibility (bool), stored in `db.global.minimap`, which is **LibDBIcon's own table** (`launcher-§3`). Global rather than profile on purpose: a profile switch must not move a player's buttons and the options-ui-§12 reset must not un-hide one. The row's sense is **inverted** against the stored key — the label says shown, `hide` says hidden — so its `get`/`set` negate at the single write seam.
