...
| `core/Database.lua` | `NS.Database` | `SCHEMA_VERSION`, `global.schemaVersion` default, `RunMigrations(db)` (`SCHEMA_VERSION` 2: one step, v2, lifts a Loot-only `LOOT_ITEM_CREATED_SELF[_MULTIPLE]` override onto Tradeskill — PRETTYCHAT-R-02; a profile-scoped step runs on every stored profile, and the stamp advances only past a step that succeeded) and the load-pass repair `PruneOrphans(db)`, which it runs last. |
