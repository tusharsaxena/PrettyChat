...
`/pc` and `/prettychat` dispatch through one ordered `COMMANDS` table in `settings/Slash.lua` (help text is generated from the same table). Verbs: `help`, `config`, `version`, `list`, `get`, `set`, `reset`, `resetall`, `test`, `debug`, `enable`, `disable`. The last two are the reserved **aliases** (`slash-commands-§2`): they write `General.enabled` — the Master-controls *Enable PrettyChat* row's own stored path — through the same `NS.Schema.Set` seam the checkbox writes through, and hold no state of their own. `/pc` keeps working while the addon is disabled, so the pair is never one-way: `OnInitialize` registers the chat command unconditionally and nothing unregisters it. **The disabled surface is the full reserved set** (`slash-commands-§2`, `§7`): `help`, `config`, `version`, `enable`, `disable`, `debug`, `perf` and the schema CLI (`get`, `set`, `list`, `reset`, `resetall`) all answer normally, and the **bare `/pc` opens the settings panel** — the case that settled the rule when v2.56.0's narrowing was reversed at v2.57.0. What a disabled addon *does* refuse is a verb that drives its FEATURES: one tagged line naming `/pc enable`, and nothing else. **The gate is the library's** (`LibKa0s-Slash-1.0`, the vendored minor — 15 as of LibKa0s v1.56.0): the descriptor passes `isEnabled` (asked at dispatch time, never cached) and `brandName`, the live set is `lib.LIVE_VERBS` and the wording is `lib.DISABLED_LINE_FORMAT`, so neither is restated here. No `liveVerbs` is passed — that field *widens* the set, and this addon wants its one feature verb refused. Today that is exactly one verb, `test`; the General page's Test button is not a verb and is not gated. A bare `/pc` runs `config`, which opens the parent settings page; `/pc help` prints the command list (slash-commands-§4). `NS.COMMANDS` is published as positional triples so the landing page renders the same list through `Sl:LandingRows()` — one formatter, two surfaces. Dispatch, help, the schema verbs, the `key = value` pair and the parser are `LibKa0s-Slash-1.0`'s; this addon adds a `format` hook (the `||` doubling) and a `parse` hook (the `||` unescape on the way in; the library keeps a free-text value whole). `reset` takes a **path**, not a category. Chat input requires `||` for a literal `|`. Detail: [slash-dispatch.md](./slash-dispatch.md).
