...
- `settings/OptionsSetup.lua` sits after `settings/Schema.lua`, whose runtime members (`NS.SchemaRuntime.Get`/`Set`/`ApplyDefault`/`AllRows`, taken as values at file load) and `RowsByCategory` its descriptor reads, and before `settings/Panel.lua`, which takes the instance as a file-scope upvalue and registers its pages at file load (options-ui-§1).

### The shared cause clause

`core/CoreSetup.lua` publishes **`NS.LIBKA0S_MISSING`** — *"The LibKa0s library is missing from this installation of Ka0s Pretty Chat (expected in libs/LibKa0s)"* — outside its own library-absent branch, because the three later seams read it on **both** paths. Each appends its own consequence and its own terminal punctuation: `", so the debug console window is unavailable."`, `", so the settings panel is unavailable."`, `", so the settings CLI is unavailable."`, and Core's own fallback printer announces once, on the first line the addon prints, with `"; running on reduced built-in fallbacks."`. The schema runtime's stub (`settings/Schema.lua`) carries a fifth tail, `", so the schema was not checked."`, for its `Validate`, which nothing in the addon calls at runtime. This is a **cross-file contract five seams depend on**, not an implementation detail of one file: a degraded install says the same thing about *why* at every site and a different thing about *what* at each one, across every Ka0s addon a user has open.
...
| `core/Database.lua` | `NS.Database` | `SCHEMA_VERSION`, `RunMigrations(db)` (`SCHEMA_VERSION` 2: one step, v2, lifts a Loot-only `LOOT_ITEM_CREATED_SELF[_MULTIPLE]` override onto Tradeskill — PRETTYCHAT-R-02; a profile-scoped step runs on every stored profile, and the stamp advances only past a step that succeeded) and the load-pass repair `PruneOrphans(db)`, which it runs last. |
...
| `core/CoreSetup.lua` | `NS.Print`, `NS.Format`, `NS.LIBKA0S_MISSING`, `NS.MakeCloseButton`, `NS.Util.SafeToString`, `NS.Util.IsConcatSafe`, `NS.Util.SafeRegisterEvent` / `SafeRegisterUnitEvent` / `SafeRegisterEvents` | The `LibKa0s-Core-1.0` seam. Builds the cyan `[PC]` printer from the descriptor (`prefix` as a function, `sep = ""` because the tag carries its own trailing space), reclaims `NS.Print` from AceConsole's embed, and owns the library-absent fallbacks. |
| `core/DebugLogSetup.lua` | `NS.DebugLog`, `NS.Debug` | The `LibKa0s-DebugLog-1.0` seam. Supplies the frame-name prefix, the title, the mono font, the `isEnabled`/`setEnabled` pair over `NS.State.debug`, the `[Init]` session summary, the visibility callback, and the diagnostics report's `brandName` and `diagnostics` field (the sections are `modules/Diagnostics.lua`'s); the window, both formatters, the buffer, the scroll sync and the `SetEnabled` seam are the library's. |
| `core/LauncherSetup.lua` | `NS.Launcher` | The `LibKa0s-Launcher-1.0` seam (`launcher-§1`). Builds the **one** LibDataBroker-1.1 object — `type = "launcher"`, the addon's own 128×128 logo, the label a broker display prints — and hands it to LibDBIcon-1.0, so the minimap button and a broker row are one object registered twice rather than two features. Supplies the FOLDER name (LibDBIcon keys the button's saved position by it), the icon path, `openSettings` (`PrettyChat:OpenConfig`, the one combat-gated open) and a **function** answering `db.global.minimap`, resolved at `Register` time because AceDB has not run when this file loads. **Both buttons are the library's** (Launcher minor 4, `launcher-§2` as of standard v2.67.0): left-click opens the settings panel in either state, right-click opens the client's context menu. The menu has **one entry, Enabled**, which is the standard's `ADDONS.md` row for this addon: the descriptor passes `isEnabled` (`PrettyChat:IsAddonEnabled()`) and `setEnabled`, which calls `NS.SetAddonEnabled` — the very function `/pc enable` and `/pc disable` call, published by `settings/Slash.lua` — so the menu is the same write through the same seam with the same echo. The addon is frameless, so it passes no lock, test-mode or window pair. **The hover tooltip is the library's** (Launcher minor 3, `launcher-§1`), drawn enabled or disabled: `Ka0s Pretty Chat  v<version>`, `Enabled: Yes|No`, `Left-click: Open settings`, `Right-click: Options menu`, from `version` (the TOC's `## Version`, through `NS.Version()`) and `isEnabled`. It passes no `onTooltipShow` (no line of its own), and none of the retired `onClick` / `leftClickLabel` / `disabledLine`. **It has no degradation stub and needs none**: its only two call sites (`OnEnable`'s `Register`, and the `global.minimap.shown` row's `set`) both guard on `NS.Launcher`, and the row reads and writes the store either way. `Register()` is called from `PrettyChat:OnEnable`, not here. |
...
| `defaults/Profile.lua` | `NS.ProfileDefaults` | The one default declaration site (savedvariables-§2): the AceDB `profile` defaults table (`{ profile = { categories = {} } }`), `NS.GlobalDefaults` (the AceDB `global` defaults: `schemaVersion = 0`, LibDBIcon's `minimap`) and `NS.GeneralDefaults` (`enabled = true`, `visibility = "always"`). `OnInitialize` assembles the first two before `AceDB:New`. |
...
| `modules/Override.lua` | `NS.RenderSample`, `NS.OriginalFormat` | The override engine — `ApplyStrings`, the enable-cascade predicates, `ResetString` / `ResetCategory` / `ResetCategoriesPage` (all three through `Schema.ResetRows`) / `ResetAll`, the Test / sample renderer, and the one reader of Blizzard's pristine format (`OriginalFormat`), shared with the panel's Original box. `ResetAll` is a **profile reset** (`options-ui-§12`): one `db:ResetProfile()`, with the re-apply and the one `[Set] reset profile '<name>' to defaults (N rows)` line landing on `core/PrettyChat.lua`'s `OnProfileReset` handler — the same reload a profile switch takes. |
| `modules/Diagnostics.lua` | `NS.Diagnostics` | The sections of the diagnostics report (`debug-logging-§14`), which `core/DebugLogSetup.lua` hands `LibKa0s-DebugLog-1.0` through the descriptor's `diagnostics` field: the master switch, latch and apply gate, the combat watcher (read through `PrettyChat.CombatWatchState`), the settings that differ from their defaults (format strings with `\|` doubled, never stripped), the live-global audit, the snapshot, patch drift, the render check and the would-apply counts, the settings pages and launcher, and any known chat-rewriting addon that is loaded. A read only: it writes no global, no setting, no registration and no hold. |
| `settings/Schema.lua` | `NS.Schema`, `NS.SchemaRuntime`, `NS.SchemaLib` | Builds `rows` from `NS.Defaults` and makes the `LibKa0s-Schema-1.0` runtime over them (or the host's write-completing, log-silent stub with LibKa0s absent); single write path (`Schema.Set`, the runtime's `Set`) and its batched entry (`ResetRows`), `AllRows`, `ApplyDefault`, `FormatValue`, load-time path validator and duplicate-registration check (`Schema.duplicateGlobals`), and the `NotifyPanelChange` fan-out that drives **both** refresher registries. |
...
| `settings/Slash.lua` | `NS.COMMANDS`, `NS.SlashCommands`, `NS.SetAddonEnabled` | The ordered `COMMANDS` table (positional triples, the host's), the `LibKa0s-Slash-1.0` descriptor with its `format` and `parse` hooks, and the five host-owned verbs: `list`'s sub-keywords and category filter, `reset`'s category-misuse redirect, `resetall`, `test`, `debug`. |
| `settings/Panel.lua` | `NS.Config.RegisterPanels`, `NS.Config.BuildMain` | The page **bodies** and nothing else, for the addon's three kinds of page: the `General` page (`H.RenderTabbedSchema` over its one composed `Master controls` tab, closed by the composer's button pair — `Test` as its `leadButton`, then `Reset all settings` — which the group's `afterGroup` hook hands straight to `Schema.masterAfterGroup`), the `Categories` page (an `H.TabStrip` of the eight message categories, a footnote, then per tab a library-made Enable row, an AceGUI `TreeGroup` — that category's format strings in its tree pane beside **one** editor for the selected string), and the landing page (a spec — logo, tagline, one `Slash Commands` section — handed to the library's `H.BuildLandingPage`, which owns the body and, at `OptionsWidgets.lua:344`, the `OnRelease` that keeps the logo texture out of AceGUI's shared frame pool). The primary strip is the library's; the tree and the editor beside it are the `options-ui-§13` and `§6` deviations. The canvas factory, the header, the Defaults button, the scroll container and the page registry are the library's. |

Topic detail: [module-map.md](./module-map.md).

## Namespace publishing pattern

Every file opens by destructuring the two values WoW passes each chunk — the addon FOLDER name and the addon-wide namespace table. Nine files read both and spell the header `local addonName, NS = ...`: `core/Namespace.lua`, `core/CoreSetup.lua`, `core/EnvSetup.lua`, `core/MediaSetup.lua`, `core/DebugLogSetup.lua`, `core/PrettyChat.lua` (which hands the folder name to `:NewAddon`), `core/LifecycleSetup.lua` (which names the latch's owner in both of its descriptors), `core/LauncherSetup.lua` (which builds the broker object's `ICON_PATH` and its `name` from it) and `settings/Panel.lua` (which builds `Interface\\AddOns\\<folder>` from it). The other twelve never read the folder name and open `local _, NS = ...` — `M4c-06` corrected them when the blanket `211/addonName` suppression that had been hiding them came out of `.luacheckrc`. A copied header naming a value the file does not use is dead code, not a convention. Modules publish their public surface onto `NS`; nothing is exported through a global. The addon object **is** that same `NS` table (`core/PrettyChat.lua` passes `NS` to `:NewAddon`, architecture-§2), so the AceAddon methods hang off it and `LibStub("AceAddon-3.0"):GetAddon("PrettyChat")` returns the very same table.
...
| `NS.Database` | `core/Database.lua` | `core/PrettyChat.lua` (`OnInitialize` and the three profile callbacks run migrations) |
| `NS.LIBKA0S_MISSING` | `core/CoreSetup.lua` | `core/DebugLogSetup.lua`, `settings/OptionsSetup.lua`, `settings/Slash.lua`, `settings/Schema.lua` (the schema runtime stub's `Validate` line) — the shared cause clause every degraded seam appends its own consequence to |
| `NS.Launcher` | `core/LauncherSetup.lua` | `core/PrettyChat.lua` (`OnEnable` calls `Register`) and `settings/Schema.lua` (the `global.minimap.shown` row's `set` calls `SetShown`) — **two callers, both guarded on the member being non-nil**, which is why this seam ships no stub |
...
| `NS.SchemaRuntime` / `NS.SchemaLib` | `settings/Schema.lua` | `settings/OptionsSetup.lua` and `settings/Slash.lua` (the descriptors take the runtime's `Get` / `Set` / `ApplyDefault` / `AllRows` / `FindRow` as values). `NS.SchemaLib` is the `LibKa0s-Schema-1.0` library or the host stub, and has no host caller: `tests/test_surface_parity.lua` reads it to pin the stub's surface |
...
- **Single write path.** Every settings mutation goes through `NS.Schema.Set(path, value)`, which is `LibKa0s-Schema-1.0`'s instance `Set` bound under the host's name: the panel widget callbacks (`settings/Panel.lua`), `/pc set` and `/pc reset <path>` (`settings/Slash.lua`). The Categories-page, per-category and per-string resets (`PrettyChat:ResetCategoriesPage` / `:ResetCategory` / `:ResetString` in `modules/Override.lua`) take its batched entry, `Schema.ResetRows(rows, label)`, which writes each row's default through the same `row.set` behind the same gates. `Set` runs `PrettyChat:ApplyStrings()` and `Schema.NotifyPanelChange()` once per write, through the runtime's `announce`, `ResetRows` once per batch, and together they keep panel and slash from drifting. Never write `db.profile.categories[…]` directly from outside a row's `set()` closure. The one other writer is `PrettyChat:ResetAll`, the options-ui-§12 profile reset (`db:ResetProfile()`), which architecture-§5 exempts as a wholesale replacement.
...
`NS.Defaults` is the source data; `settings/Schema.lua` turns it into an ordered `rows` list keyed by dot path. Seven row kinds:
...
- `global.minimap.shown` — the minimap button's visibility (bool), stored as `db.global.minimap.hide` (the path names the row's sense, the store keeps the library's key: WS-06, anti-pattern #81; no migration), in `db.global.minimap`, which is **LibDBIcon's own table** (`launcher-§3`). Global rather than profile on purpose: a profile switch must not move a player's buttons. **And it survives a reset as a *property* of the setting rather than as a consequence of that scope** (`launcher-§3`): a minimap-button choice is a per-installation display preference, like the angle the player dragged the button to, so neither options-ui-§12's *Reset all settings* nor a page-scoped **Defaults** button may un-hide it or re-hide a shown one. **Neither reaches it here** — the global reset is `db:ResetProfile()` and this addon has a real `profile` section for it to empty, the General page's **Defaults** button is bound to that same reset-all path (`PrettyChat:ConfirmResetAll` → `ResetAll`, `options-ui-§12`), and `PrettyChat:ResetCategory('General')`, which no button calls any more, names its two profile rows one by one rather than walking the category. No exemption is needed and none is declared; `tests/test_launcher.lua` drives both resets and reads the store back. The row's sense is **inverted** against the stored key — the label says shown, `hide` says hidden — so its `get`/`set` negate at the single write seam.
...
`/pc` and `/prettychat` dispatch through one ordered `COMMANDS` table in `settings/Slash.lua` (help text is generated from the same table). Verbs: `help`, `config`, `version`, `list`, `get`, `set`, `reset`, `resetall`, `test`, `debug`, `diagnostics`, `enable`, `disable`. The last two are the reserved **aliases** (`slash-commands-§2`): they write `General.enabled` — the Master-controls *Enable PrettyChat* row's own stored path — through the same `NS.Schema.Set` seam the checkbox writes through, and hold no state of their own. `/pc` keeps working while the addon is disabled, so the pair is never one-way: `OnInitialize` registers the chat command unconditionally and nothing unregisters it. **The disabled surface is the full reserved set** (`slash-commands-§2`, `§7`): `help`, `config`, `version`, `enable`, `disable`, `debug`, `perf`, `diagnostics` and the schema CLI (`get`, `set`, `list`, `reset`, `resetall`) all answer normally, and the **bare `/pc` opens the settings panel** — the case that settled the rule when v2.56.0's narrowing was reversed at v2.57.0. What a disabled addon *does* refuse is a verb that drives its FEATURES: one tagged line naming `/pc enable`, and nothing else. **The gate is the library's** (`LibKa0s-Slash-1.0`, the vendored minor — 16 as of LibKa0s v1.60.0): the descriptor passes `isEnabled` (asked at dispatch time, never cached) and `brandName`, the live set is `lib.LIVE_VERBS` and the wording is `lib.DISABLED_LINE_FORMAT`, so neither is restated here. No `liveVerbs` is passed — that field *replaces* the set rather than adding to it, and this addon wants its one feature verb refused. Today that is exactly one verb, `test`; the General page's Test button is not a verb and is not gated. A bare `/pc` runs `config`, which opens the parent settings page; `/pc help` prints the command list (slash-commands-§4). `NS.COMMANDS` is published as positional triples so the landing page renders the same list through `Sl:LandingRows()` — one formatter, two surfaces. Dispatch, help, the schema verbs, the `key = value` pair and the parser are `LibKa0s-Slash-1.0`'s; this addon adds a `format` hook (the `||` doubling) and a `parse` hook (the `||` unescape on the way in; the library keeps a free-text value whole). `reset` takes a **path**, not a category. Chat input requires `||` for a literal `|`. Detail: [slash-dispatch.md](./slash-dispatch.md).
...
|---|---|---|---|
| `PLAYER_REGEN_DISABLED` | `PrettyChatCombatWatcher` (`modules/Override.lua`) | only while the addon is **stood up** AND `General.visibility` is `inCombat` or `outOfCombat` | one `ApplyStrings` pass + one `[Visibility]` summary line |
...
**Disabled means this addon is not running** — not hidden, not quiet, not skipping a repaint (`slash-commands-§7`). Until the latch landed, *disabled* here was a **draw gate**: `IsAddonEnabled()` was one rung of `ApplyStrings`' ladder, so the Blizzard originals came back and nothing else changed. `PrettyChatCombatWatcher` stayed registered for `PLAYER_REGEN_DISABLED` / `PLAYER_REGEN_ENABLED` whenever a combat-scoped visibility was stored, and every combat boundary went on waking the addon up to walk 79 globals and decide to restore them again, on an addon the player had switched off. From outside that is indistinguishable from standing down, which is how the shape survived every audit (anti-pattern #85).

**One latch, two named holds.** `core/LifecycleSetup.lua` publishes `NS.Lifecycle`, a `LibKa0s-Lifecycle-1.0` instance. `disabled` is taken from the stored `General.enabled` path — at load in `OnEnable`, from the row's `set` (so the checkbox, `/pc enable` / `/pc disable` and `/pc set General.enabled false` all drive it), and re-evaluated on AceDB's three profile callbacks, because a profile switch can flip the path with nothing else touched. `perf` is the hold `LibKa0s-Perf-1.0` takes for a capture's suspended arm; **this addon never takes it**, under the recorded `performance-§12` exemption below. The latch is wired anyway, because the point of it is that **releasing one hold must not stand up an addon the other is still holding down** — and `/pc enable` is a live verb, so a player can hit it mid-capture.

**What stands down.** The combat watcher's two event registrations, actually **unregistered** — `SyncCombatWatch` reads the latch as its *first* term, so the ladder answers no **at the source** and a later visibility change cannot re-arm it behind the switch's back. And every `_G[GLOBALNAME]` override restored to this client's pristine value, which for a frameless addon whose display *is* the chat text is the whole of "nothing is drawn". There is nothing else: no AceTimer, no `C_Timer` ticker, no `OnUpdate`, no message, no bucket, no hook of any kind, and no secure or attribute work — so `§7`'s combat-lockdown carve-out has nothing to defer, and no `PLAYER_REGEN_ENABLED` completion is held pending.

**What survives, because it is SETUP and not a feature.** The `/pc` and `/prettychat` chat commands and the whole dispatcher; the settings-category registration and its panel body; the AceDB handle, the single write seam and the three profile callbacks; and the launcher's broker object and minimap button. Neither launcher button is refused (`launcher-§2`, Launcher minor 4): left-click opens the settings panel, which `§7` itself keeps standing, and right-click opens the options menu, whose **Enabled** entry stays live so the addon can be switched back on from it. Opening either writes no SavedVariables. The hover tooltip stays too, and reads `Enabled: No`.

**The seam is one mechanism, not two.** `PrettyChat.Reapply` is both latch arms: it asks the state as it is *now* and makes the client agree, which is the honest implementation of going down and of coming back up. A second teardown path beside it would be two mechanisms that must agree about what inert means, and they diverge on the first module added after the second one is written. `standUp` therefore rebuilds **from current state**, never from a snapshot — a visibility changed while the addon was off is honored the moment it comes back (`performance-§6`).

The conformance suite is `tests/test_disabled.lua` (`§7`). Every assertion is made against the **registration set**, through the kit's recording mock, and never against a handler's return value: a suite that called a handler and asserted it returned early would certify the draw gate it exists to catch.
...
- `SetRenderer` puts the **combat lock** on the page itself, because the Blizzard AddOns sidebar reaches a panel without going through the panel-open at all — the path a user is most likely to take mid-fight. Since LibKa0s v1.46.x (options-ui-§2, anti-pattern #88) a page shown in combat is **covered, not closed**: nothing renders, nothing of Blizzard's settings window is touched, one gray `COMBAT_LOCKED_NOTICE` line is printed per combat, and writes / Defaults / tab switches are refused until `PLAYER_REGEN_ENABLED` draws the page. The lock is the library's alone — this addon has no host-side combat guard on the settings surface.
- No `SecureHook`, no protected-frame creation, no combat-sensitive writes beyond the guarded panel open and the library's combat lock.
...
**Audit and review history.** The most recent compliance audit is [audits/2026-09-23/](./audits/2026-09-23/)
(audited against Standard **v2.64.0**); the most recent engineering review is
[reviews/2026-09-23/](./reviews/2026-09-23/). Earlier audit bundles are kept for history:
[2026-09-08](./audits/2026-09-08/) (Standard **v2.39.0**), [2026-09-07](./audits/2026-09-07/) (Standard **v2.38.0**), [2026-08-05](./audits/2026-08-05/)
...
|---|---|---|---|---|
| `performance-§12` | No perf harness is wired: no `core/PerfSetup.lua`, no `PrettyChatPerfDB`, no `perf` verb registration, no `tests/perf.lua`, no `docs/perf-analysis/`. The **stand-down capability exists** (`NS.Lifecycle`, above) — what is absent is a probe to take the `perf` hold, so the latch carries exactly one hold in production | **Criterion (a) holds**, proven by the committed whole-repo sweep in [`performance-sweep.md`](./performance-sweep.md), last re-taken in the `PC-23:` commit. **Both (b) and (c)** apply, as [`performance.md`](./performance.md) states. Reasoned at length as [`LIBKA0S-12`](https://github.com/tusharsaxena/PrettyChat/issues/10) | 2026-08-05, re-checked 2026-09-02, 2026-09-08 (the 2026-09-02 sweep predated the settings-panel timer by one day) and 2026-09-24 (PC-23, on the LibKa0s v1.56.0 tree) | **The first `OnUpdate` handler, repeating ticker, or event handler that runs DURING combat rather than at its boundary re-arms the full wiring MUST.** Re-run the sweep in `performance-sweep.md`: it now returns the watcher and the settings panel's next-frame fit, and the question to ask of any new hit is whether it fires while the player is fighting |
| `layout-§2` | `GlobalStrings/` is a **PascalCase, root-level** folder outside the mandated `core/ defaults/ settings/ locales/ modules/` skeleton (`layout-§1`), holding 26 machine-generated chunks (~22,879 Blizzard reference strings) plus the 23,842-line source dump `GlobalStrings/GlobalStrings.lua` | The modular layout has no home for bulk generated reference data. **Nothing ships and nothing loads:** since PC-R-05 the whole folder is `.pkgmeta`-ignored and `PrettyChat.toc` carries no chunk line, so it is repo-only reference data that `tests/test_defaults.lua` reads to check every override against Blizzard's real signature. Chunks stay cut by entry count, not by first letter, and each is under 1000 lines so the generated data stays out of `layout-§1`'s on-notice band (PC-49). Regenerate with `python3 tools/split_globalstrings.py` (the generator moved to `tools/` under `layout-§1`, "Where an authored generator lives"; its output stays here), which now REFUSES to run if the TOC has started loading the chunks again. **What is left in this row is the folder's NAME and PLACE, and nothing else.** It used to carry the dump's 1500-LOC breach too; `layout-§1` was revised on 2026-09-08 (standard v2.39.0) to cap generated non-shipping data at its generator rather than at its output, so the dump is exempt **by rule** and is recorded under *Files over the 1500-line cap* below rather than deviating here | 2026-08-05, cap half retired 2026-09-08 | **The old trigger has half fired and is replaced.** What ends this row now is a `layout-§2` revision permitting a PascalCase non-source folder at the root, **or** the folder moving under `tests/` now that it is a fixture rather than shipped data — which would settle the casing and the placement in one move. The `toc-file-§5` half of this pair was already closed; the cap half closed with the revision |
| `toc-file-§1` | `## Title:` keeps its rainbow `\|cRRGGBB…\|r` escapes and `## Author:` keeps its stylized `aDd1kTeD2Ka0s` casing | The color escapes and the casing are the addon's brand mark, kept deliberately rather than normalized to `Ka0s Pretty Chat` / `add1kted2ka0s` | 2026-07-12, narrowed 2026-09-24 | A decision to retire the brand mark |
...
**Re-checked against the generator's move to `tools/` (2026-09-24, `PRETTYCHAT-A-07`).** The
...
Vendored under `libs/` (the BigWigs packager pulls nothing — no `externals`): LibStub, CallbackHandler-1.0, AceAddon-3.0, AceDB-3.0, AceConsole-3.0, AceGUI-3.0, **LibDataBroker-1.1** and **LibDBIcon-1.0** (the launcher's pair, `launcher-§1` — neither is LibKa0s's dependency; it resolves both with `LibStub(…, true)` at `Register` time and degrades by name), and **[LibKa0s](https://github.com/tusharsaxena/LibKa0s) v1.61.0** (`libs/LibKa0s/`, listed in the TOC as `libs\LibKa0s\LibKa0s.xml` after Ace3). (`AceConfig-3.0` was removed — no live consumer.)
...
The debug console's font is fixed monospace on purpose (JetBrains Mono, reaching `LibKa0s-DebugLog-1.0` as `Const.FONT_MONO`): aligned log output reads the same whatever face a player prefers elsewhere, so it stays that way until the first user-facing media picker gives LibSharedMedia a consumer here.
...
- **`.gitignore`** covers OS/editor cruft and `.claude/`. `libs/` is tracked (vendored Ace3), as are `GlobalStrings/`, `tools/` (the chunk splitter), `media/`, all `.lua` source, `tests/`, `.luacheckrc` and `.pkgmeta`.
