...
| `NS.Launcher` | `core/LauncherSetup.lua` | `core/PrettyChat.lua` (`OnEnable` calls `Register`) and `settings/Schema.lua` (the `global.minimap.hide` row's `set` calls `SetShown`) — **two callers, both guarded on the member being non-nil**, which is why this seam ships no stub |
