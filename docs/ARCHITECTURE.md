...
- **Single write path.** Every row write from the panel widget callbacks (`settings/Panel.lua`), `/pc set` and `/pc reset <path>` (`settings/Slash.lua`) goes through `NS.Schema.Set(path, value)`. It runs `PrettyChat:ApplyStrings()` and `Schema.NotifyPanelChange()`, and it is what keeps panel and slash from drifting. Never write `db.profile.categories[…]` directly from outside a row's `set()` closure. Two existing verbs already do: `PrettyChat:ResetCategory` and `PrettyChat:ResetString` (`modules/Override.lua`) clear row-backed storage directly and run the re-apply and the refresh themselves. That is an open architecture-§5 gap, not a documented deviation ([Settings Schema](#settings-schema)). Do not add a third.
...
Every row write from `/pc set`, `/pc reset <path>` and the panel widgets goes through `NS.Schema.Set(path, value)`, the **single write path**. The two reset verbs named below are the exception. Row `set()` closures are pure DB writes; `Schema.Set` runs `PrettyChat:ApplyStrings()` + `Schema.NotifyPanelChange()`, and, being the seam every value write crosses, owns the conversion-signature gate that refuses a dangerous format before it is stored (returns `false`, refreshes the panel so the box shows what actually **is** stored). `string_format` rows **auto-clear** on a default match (a value equal to the default deletes the stored override). At load, `Schema.validation` records that every row path resolves to a backing default (loud `NS.Print` warn on any miss). Settings persist in `PrettyChatDB` via AceDB on a single shared Default profile; the `profile` defaults come from `defaults/Profile.lua` (`NS.ProfileDefaults`) and `db.global.schemaVersion` is stamped by `Database.RunMigrations`. Detail: [schema.md](./schema.md).

**Structural registries (architecture-§5): none.** Storage keys: none qualify. `db.profile.categories` is keyed by the fixed `Schema.CATEGORY_ORDER` names, and each category's `strings` / `disabledStrings` by the fixed Blizzard GLOBALNAME set in `NS.Defaults`. The player never adds or removes a member, and every stored leaf has a row path, so no collection passes the three-part test. Writer: none is needed. `PrettyChat:EnsureCategoryDB` (`modules/Override.lua`) only creates an empty category table on the first write from a row's `set()` closure, which is a traversal accessor, not a writer. Load pass: `Database.RunMigrations` (`core/Database.lua`), called from `OnInitialize` and from the `OnProfileChanged` / `OnProfileCopied` / `OnProfileReset` callbacks in `core/PrettyChat.lua`. It stamps `db.global.schemaVersion`, which is bookkeeping and not a row, and with `migrations` empty it writes nothing else. `PrettyChat:ResetAll` is the options-ui-§12 profile reset (`db:ResetProfile()`), a wholesale replacement and not a registry write.

**Remaining architecture-§5 gap: two resets write row paths around the helper.** `PrettyChat:ResetCategory` (the per-category Defaults button) sets `db.profile.enabled` and `db.profile.visibility` to nil for `General`, and `db.profile.categories[<Category>]` to nil for any other category. `PrettyChat:ResetString` (the per-string Reset button) clears `strings[NAME]` and `disabledStrings[NAME]`. Each backs schema rows. Both run `ApplyStrings` and `Schema.NotifyPanelChange` themselves and log one `[Reset]` summary. No `Documented deviations` row covers them, so this is an open gap tracked as [tusharsaxena/PrettyChat#15](https://github.com/tusharsaxena/PrettyChat/issues/15), not a ratified deviation. It closes either through a batched entry on the helper or through a register row. Compliant paths: `/pc reset <path>` and the library's `applyDefault` hook (wired in `settings/Slash.lua` and `settings/OptionsSetup.lua`) go through `Schema.ApplyDefault` → `Schema.Set`, and bare `/pc reset <category>` only prints guidance.
...
Vendored under `libs/` (the BigWigs packager pulls nothing — no `externals`): LibStub, CallbackHandler-1.0, AceAddon-3.0, AceDB-3.0, AceConsole-3.0, AceGUI-3.0, and **[LibKa0s](https://github.com/tusharsaxena/LibKa0s) v1.30.0** (`libs/LibKa0s/`, listed in the TOC as `libs\LibKa0s\LibKa0s.xml` after Ace3). (`AceConfig-3.0` was removed — no live consumer.)
