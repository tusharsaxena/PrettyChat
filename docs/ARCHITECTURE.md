...
**Remaining architecture-§5 gap: two resets write row paths around the helper.** `PrettyChat:ResetCategory` (the per-category Defaults button) sets `db.profile.enabled` and `db.profile.visibility` to nil for `General`, and `db.profile.categories[<Category>]` to nil for any other category. `PrettyChat:ResetString` (the per-string Reset button) clears `strings[NAME]` and `disabledStrings[NAME]`. Each backs schema rows. Both run `ApplyStrings` and `Schema.NotifyPanelChange` themselves and log one `[Reset]` summary. No `Documented deviations` row covers them, so this is an open gap tracked in the issue store, not a ratified deviation. It closes either through a batched entry on the helper or through a register row. Compliant paths: `/pc reset <path>` and the library's `applyDefault` hook (wired in `settings/Slash.lua` and `settings/OptionsSetup.lua`) go through `Schema.ApplyDefault` → `Schema.Set`, and bare `/pc reset <category>` only prints guidance.
