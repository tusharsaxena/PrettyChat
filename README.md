...
Waiting on real loot is a poor way to judge a wording change, so you can ask for a sample instead. `/pc test` writes every message twice to the debug console, the game's version above yours. That keeps several hundred lines out of your chat window, and the **Test** button on the **General** page does the same run. Narrow it with `/pc test category Loot` or `/pc test formatstring LOOT_ITEM_SELF` when the full run is more than you wanted. One difference while the addon is switched off: the button still works, but `/pc test` just tells you to `/pc enable` first. Mid-edit you want neither: the settings panel carries the same before-and-after as a **Preview** box that updates while you type.
...
Three switches decide whether your version is used, checked in that order: the master **Enable PrettyChat**, then the category, then the message. Switch any one off and Blizzard's original comes back while your wording sits there waiting. The master has chat equivalents — `/pc disable` switches PrettyChat off, `/pc enable` switches it back on. The master lives on the **General** page with the **General visibility** dropdown beside it (*Always*, *Only in combat*, *Only out of combat*, *Never*), which is how you get PrettyChat during a fight and stock chat the rest of the time. Undo works at whatever scale you need: **Reset** on one message, **Defaults** on the Categories page for every category tab at once, **Reset all settings** (or the General page's own **Defaults** button, which is the same reset) for the lot. PrettyChat also puts a button on your minimap, wearing its own logo: left-click opens the settings, right-click opens a small menu with an **Enabled** tick box that switches PrettyChat off and on (the same switch as `/pc disable` and `/pc enable`), hovering it shows the version and whether PrettyChat is on, and the **Minimap button** checkbox on the **General** page takes it away again if you would rather it were not there. If you run Titan Panel, Bazooka or ElvUI's data texts, the same button turns up there too. And if something is wrong rather than merely unwanted, `/pc debug` opens a session-only log window, and [Reporting a bug](#reporting-a-bug) below says what to do with it.

Everything else is configuration, and it lives in two places: the addon's own page under Settings → AddOns in game, and `/pc` (or `/prettychat`), which opens that page. `/pc help` prints the full command list, and `/pc version` prints the version you are running.
...
| How do I preview my edits without waiting for real loot? | Two ways. On the settings panel, each message has a live **Preview** that updates as you type. From chat, `/pc test` writes a before/after sample of every message to the debug console — add a category (`/pc test category Loot`) or one string (`/pc test formatstring LOOT_ITEM_SELF`) to narrow it down. While the addon is switched off, `/pc test` asks you to turn it back on; the Preview and the General page's **Test** button keep working. |
...
| What's the **Debug console** for? Do I need it? | No. It's a troubleshooting aid for when you're filing a bug, nothing more. `/pc debug` (or the General-page toggle) opens a small on-screen log window. It starts empty because logging is off by default. [Reporting a bug](#reporting-a-bug) says how to turn it on and copy out what it caught. Logging is session-only and resets every reload. |
...
| I want a clean slate | One message: its **Reset** button, or `/pc reset path` (`/pc list` shows the paths). Every category's strings: the Categories page's **Defaults** button, which resets every category tab, not only the one you are looking at. Everything: `/pc resetall`, or **Reset all settings** or the **Defaults** button on the General page (both ask first). |
| I opened the debug console but it's empty | The window and logging are separate switches. Opening the window (`/pc debug`) doesn't start logging. Turn logging on first with `/pc debug on`, or the **Debug** toggle inside the window, then reproduce the problem. To send it in, follow [Reporting a bug](#reporting-a-bug) below. |
